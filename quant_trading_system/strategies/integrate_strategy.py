...
────────────────────────────────
사용법:
  python strategies/integrate_strategy.py <새_전략_파일_경로>

예시:
  python strategies/integrate_strategy.py strategies/mean_reversion_strategy.py

이 스크립트는 새로 작성한 전략 파일의 TASK_META를 읽어
아래 파일들을 자동으로 수정합니다:
...
전제 조건:
  1. 새 전략 파일에 TASK_META 딕셔너리가 정의되어 있어야 함
  2. TASK_META 양식은 TEMPLATE_NEW_STRATEGY.py 참고
  3. main_scheduler.py, settings.py에 통합 마커 주석이 존재해야 함
"""
...
def validate_meta(meta: dict):
    ...
    if meta["type"] != "strategy":
        raise ValueError(
            f"TASK_META의 type이 'strategy'이어야 합니다 (현재: {meta['type']}). "
            f"스크래퍼라면 scrapers/integrate_scraper.py를 사용하세요."
        )
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        print("사용법: python strategies/integrate_strategy.py <새_전략_파일_경로>")
        print("예시:   python strategies/integrate_strategy.py strategies/mean_reversion_strategy.py")
        sys.exit(1)
