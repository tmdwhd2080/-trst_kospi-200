"""
[stock_scrapping] 장중 주가 OHLCV 수집
──────────────────────────────────────────
KIS API 기반 장중 OHLCV 수집 스크래퍼.

- 스케줄러 진입점: run()
- 즉시 시작 판단: should_start_now()
- CLI/내부 실행기: run_poller()

이 파일은 TEMPLATE_NEW_SCRAPER.py 형식의 TASK_META를 포함하므로
integrate_scraper.py에서 읽을 수 있습니다.
"""

from __future__ import annotations

TASK_META = {
    "task_id": "stock_scrapping",
...
def _resolve_output_path() -> Path:
    ...
    return _resolve_path(configured_output, DEFAULT_OUTPUT)


def _build_runtime_args() -> argparse.Namespace:
    return argparse.Namespace(
        stock_list=_resolve_path(getattr(cfg, "STOCK_SCRAPPING_STOCK_LIST", DEFAULT_STOCK_LIST), DEFAULT_STOCK_LIST),
        ...
        batch_size=int(getattr(cfg, "STOCK_SCRAPPING_BATCH_SIZE", DEFAULT_BATCH_SIZE)),
        poll_interval=float(getattr(cfg, "STOCK_SCRAPPING_POLL_INTERVAL_SEC", FIXED_BAR_INTERVAL_SEC)),
        min_request_gap=float(getattr(cfg, "STOCK_SCRAPPING_MIN_REQUEST_GAP_SEC", DEFAULT_MIN_REQUEST_GAP_SEC)),
        max_retries=int(getattr(cfg, "STOCK_SCRAPPING_MAX_RETRIES", DEFAULT_MAX_RETRIES)),
        cycles=0,
...
def should_start_now(now: datetime | None = None) -> bool:
    ...
    if not tracked_markets:
        return False
    return any(current_market_session(market_name, now) for market_name in tracked_markets)
