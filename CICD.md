...
1. **`quant_trading_system/scrapers/stock_scrapping.py` 신규 추가**
   - KIS API 기반 장중 OHLCV 수집기와 스케줄러 진입점을 하나의 파일로 통합
   - 종목 리스트(`stock_list.json`)를 읽어 KRX/US 종목을 세션 단위로 폴링(실시간 clock으로 개장 여부 확인)
   - 재시도/백오프, 세션 종료 후 토큰 폐기, CSV 적재 로직 포함
   - `TASK_META`를 포함해 `integrate_scraper.py`와도 호환되도록 정리
   - 봇이 장중에 재시작되어도 즉시 시작할 수 있도록 `should_start_now()` 제공

2. **`quant_trading_system/stock_list.json` 신규 추가**
   - `kospi`, `sp500` 그룹 기반 종목 메타데이터 추가
   - `stock_scrapping` 작업의 기본 심볼 유니버스로 사용

3. **`quant_trading_system/main_scheduler.py` 수정**
...
4. **`quant_trading_system/settings.py` 수정**
...
5. **`quant_trading_system/run.py` 수정**
   - 서버 배포 시 `scrapers/`와 `stock_list.json`이 함께 업로드되도록 반영

6. **`quant_trading_system\\requirements.txt` 수정**
